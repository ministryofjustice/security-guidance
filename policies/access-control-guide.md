...
- Log data should not be retained for longer than 2 years unless otherwise stipulated. Retention rules may vary on a case by case basis so should be checked with the Data Privacy Team, the Cyber Assistance team, and the MoJ Data Protection Officer if it involves personal information. See the [Accounting Guide](../security-decisions/standards/accounting.md) for further information.
- Log files should not be tampered with, for example through modification or removal, under any circumstance.

See the [Security Log Collection Guide](security-decisions/principles/security-log-collection.md) for more information.
