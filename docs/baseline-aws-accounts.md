...
|What must be in place|Monitoring|Resolution/Escalation if baseline is broken/violated|
|---------------------|----------|----------------------------------------------------|
|[GuardDuty](https://aws.amazon.com/guardduty/) is enabled on all accounts, in all regions, all of the time.|Alerts fire when GuardDuty is not enabled in a AWS account. Alerts fire for at least `HIGH` and higher \(or some version of\) GuardDuty matches.|GuardDuty is automatically re-enabled.|
...
|What must be in place|Monitoring|Resolution/Escalation if baseline is broken/violated|
|---------------------|----------|----------------------------------------------------|
|[CloudTrail](https://aws.amazon.com/cloudtrail/) is enabled within all accounts, all of the time. CloudTrail logs are carbon copied to an AWS account controlled by Cyber Security.|Alerts fire when CloudTrail is not enabled in an AWS account.|CloudTrail is automatically re-enabled.|
...
|What must be in place|Monitoring|Resolution/Escalation if baseline is broken/violated|
|---------------------|----------|----------------------------------------------------|
|[Config](https://aws.amazon.com/config/) is enabled within all accounts, all of the time. Config logs are carbon copied to an AWS account controlled by Cyber Security via CloudTrail.|Alerts fire when Config is not enabled in an AWS account.|Config is automatically re-enabled.|
