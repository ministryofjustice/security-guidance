...
It is policy to follow and comply with [HMG Email Security policy](https://www.gov.uk/guidance/securing-government-email).
...
[Sender Policy Framework \(SPF\)](https://en.wikipedia.org/wiki/Sender_Policy_Framework) be implemented for email domains. SPF enables organisations to publish a Domain Name System \(DNS\) record of all the domains and IP addresses which are trusted for sending and receiving email.
...
[Domain Keys Identified Mail \(DKIM\)](https://en.wikipedia.org/wiki/DomainKeys_Identified_Mail) be enabled for all email domains. DKIM enables automatic filtering or rejection of emails that fail DKIM verification.
...
[Domain-based Message Authentication, Reporting and Conformance \(DMARC\)](https://en.wikipedia.org/wiki/DMARC) is an email authentication standard that be used with SPF and DKIM to:
...
`DMARC TXT` records be available for [creation](https://www.dmarcanalyzer.com/how-to-create-a-dmarc-record/) or iteration across the . This is to comply with the GOV.UK DMARC configuration [guide page](https://www.gov.uk/government/publications/email-security-standards/domain-based-message-authentication-reporting-and-conformance-dmarc).
