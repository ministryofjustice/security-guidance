...
Although external tools like MailChimp have been used previously within the MoJ, the emails generated by MailChimp are not compatible with many of the MoJ email filters. The result is that the emails are flagged as spam or from spoofed originators. The preferred option is to use the [GOV.UK Notify](https://www.notifications.service.gov.uk/) service whenever possible for sending newsletters or other bulk mailings, as this is generally not subject to the same issues as external mailing services.

It has been suggested that it could be possible to use MailChimp without these issues if it were set up to be on a subdomain of `justice.gov.uk` and the `SPF/DMARC/DKIM` controls were correctly configured. However, this is non trivial and would require significant technical work to achieve, hence the preference to use [GOV.UK Notify](https://www.notifications.service.gov.uk/).
