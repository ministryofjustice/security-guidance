...
-   Incorrect disposal can lead to unauthorised access to information. Disposal of information should be done using approved processes, equipment or service providers. Refer to the [Data Destruction guide](data-destruction.md) to understand when the disposal should be witnessed and recorded.
...
|----|-------|-------|
|**`PERSONNEL`**|Make sure all staff including contractors undergo baseline security clearance checks.|A contractor working with the Security Team must undergo a baseline background check \(i.e. BPSS check\) at minimum. Refer to [Security Vetting Guidance](https://intranet.justice.gov.uk/guidance/hr/recruitment/security-vetting/).|
...
||services can be used for information.||
||You must not use removable media such as an USB memory stick unless it is unavoidable. When you have to use one, it must be issued, encrypted so that the effects of losing it are minimised, and the data erased securely after use.||
