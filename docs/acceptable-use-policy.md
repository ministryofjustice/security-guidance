...
`POL.ITAUP.007`: Any request to perform an action on an IT system which contravenes its SyOPs **SHALL** be approved by the [Security team](mailto:security@justice.gov.uk), or the MoJ Chief Information Security Officer \(CISO\), before the action is taken.
...
`POL.ITAUP.026`: Any User wishing to take a remote access device out of the UK **SHALL** consult the [Remote Working](remote-working.md) guidance before doing so, and the applicable device IT Security Operating Procedures document.
